...
MAIN_REPO="demurgos/messing-with-travis"
# Deploy only on merge commit to this branch
MAIN_BRANCH="master"
# Id in the name of the key and iv files
TRAVIS_ENCRYPTION_ID="xxxxxxxx"
...
if [[ "${CI_BUILD_TYPE}" == "tag" ]]; then
  ...
  IS_GIT_HEAD_TAG_ON_DEPLOYMENT_BRANCH=`! (git tag --merged "origin/${MAIN_BRANCH}" 2> /dev/null || true) | grep --quiet --fixed-strings --line-regexp "${GIT_HEAD_TAG}" && echo "false" || echo "true"`
fi
